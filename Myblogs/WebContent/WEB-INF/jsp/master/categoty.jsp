...
<thead>
	<form method="post" action="${pageContext.request.contextPath}/master/add_cate.action" >
    		<div>
    		<input type="submit" class="special"  style="float:right; margin-right: 30px;" value="Adding"/>
    		<input type="text" style="background:#f4f4f4; border:0px; padding:0em; width:300px; float:right;" name="content"  placeholder="category name">
      		</div>
	</form>	
    <tr>
      <th>id</th>
  ...
  <c:forEach items="${cate_list}" var="u">
  		<form method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/master/change_cate.action" >
    		<input type="text" style="background:#f4f4f4; border:0px; padding:0em; display: none;" name="id" value="${u.id}"/>
    		<tr style="background:#f4f4f4;">
    		<td>${u.id}</td>
    		<td><input type="text" style="background:#f4f4f4; border:0px; padding:0em" name="content" value="${u.content}"></td>
      		<td>
      			<input type="submit" class="special" value="修改"/>
      			<a href="${pageContext.request.contextPath}/master/delete_cate.action?id=${u.id}" style="border:0px;"><input type="button" class="special" value="删除" onclick="javascript:return confirm('确实要删除该分类吗?')"/></a>
      		</td>
    		</tr>
    	</form>
   </c:forEach>
